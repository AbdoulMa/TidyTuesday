...
plot <- medals_records %>% 
  ...
  geom_line(data = medals_records %>%
    ...
    aes(year, rk, group = abb, alpha = ifelse(abb %in% top_three, 1, .25),
                color = ifelse(abb %in% top_three, abb, NA),
                size = ifelse(abb %in% top_three, 2.5, 1.5))) +
  geom_point(aes(year, rk, color = ifelse(abb %in% top_three, abb, NA), 
                 stroke = ifelse(abb %in% top_three, 1.75, .75),
             size =  ifelse(abb %in% top_three, 11.5, 8.5),
             group = seq_along(year)
             ),
             shape = 21, fill = "white") + 
  geom_text(aes(year, rk, label = abb, group = seq_along(year),
                size =  ifelse(abb %in% top_three, 3.75, 2.75),
                color = ifelse(abb %in% top_three, abb, NA)), 
            family = "Lato Black",
            fontface = "bold") + 
  ...
  theme(
    ...
    plot.title = element_markdown(color = "black", family = "Lato Black", size = rel(2), margin = margin(t = 5,b = 10), hjust = .5),
...
animate(animation, 10*5, fps = 5 , duration = 30, width = 1200, height = 800, end_pause = 10, 
