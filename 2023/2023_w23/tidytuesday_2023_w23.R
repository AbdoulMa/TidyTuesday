...
df <-  df |> 
  filter(year == 2021, is.na(iso_code), 
...
(plot <- region_mix |> 
  ...
  geom_polygon(aes(group = country), fill = "#E90B2E", color = "black") + 
   labs(
     ...
     caption = "Tidytuesday Week-23 2023<br> Abdoul ISSA BIDA <span style='font-family: \"Font Awesome 5 Brands\"'>&#xf099;</span>**@issa_madjid**<br>
  ...
  theme(
    ...
    plot.title = element_text(family = "I Sans Semibold Pro", size = rel(3.5)),
    strip.text = element_text(family = "I Sans Semibold Pro", size= rel(0.9)),
    panel.spacing.x = unit(1, "cm"), 
    panel.spacing.y = unit(1, "cm"), 
    # panel.grid = element_line(color = "grey75", linewidth = rel(0.75)),
    axis.title = element_blank(),
    axis.text.x = element_text(size = rel(0.95)),
    axis.text.y = element_blank(),
    plot.subtitle = element_text(margin = margin(b = 0.75, unit = "cm")),
    plot.caption = ggtext::element_markdown(size = rel(1.025), margin = margin(t = 0.5, unit = "cm")),
    plot.margin = margin(c(0.5,0.125,0.5,0), unit = "cm"), 
...
ggsave(filename = glue::glue("{path}.png"), width = 8, height = 9, device = ragg::agg_png, dpi = 300)
