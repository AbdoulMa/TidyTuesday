...
big_tech_companies <- big_tech_companies |> 
  mutate(lower_stock = tolower(stock_symbol),
...
(main_plot <- ggplot() + 
  geom_line(data = big_tech_stock_prices, aes(date, adj_close, color = clr, group = stock_symbol), size = .75) + 
    ...
    geom_richtext(data = companies_logos_df, aes(x = first_date, y = max_adj_close, label = glue::glue("<img src='{company_logo_path}' width='40'/>")), 
                  hjust = 0,
                  fill = NA,
  ...
  labs(
    ...
    subtitle = "Closing Stock prices evolution between 2010 & 2022"
  ) + 
  ...
  theme(
    ...
    plot.subtitle = element_text(color = "#333333", family = "Go Bold", size = rel(1.75), margin = margin(b = 1, unit = "cm")),
...
(caption_plot <- ggplot() + 
  annotate(geom = "richtext", x = 0, y = 0, label = "Tidytuesday Week-06 2023 <br>Data from: Yahoo Finance<br>Abdoul ISSA BIDA <span style='font-family: \"Font Awesome 5 Brands\"'>&#xf099;</span>**@issa_madjid**",
...
ggsave(filename = glue::glue("{path}.png"), width = 12.5, height = 9.5, device = ragg::agg_png, dpi = 300)
