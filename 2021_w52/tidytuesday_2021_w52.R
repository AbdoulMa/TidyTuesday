...
# Maybe do the text summary before 
...
plot <- nutritions_summary %>% 
   ...
   # annotate(geom = "text", x = 1.65, y = 0.25, size = 3.5, label = "Text", hjust = 0) +
...
cowplot::ggdraw(plot) +
   ...
   cowplot::draw_label(x = .9, y = .075, label = "*Average value for 100 ml of beverage \nData from Official Starbucks Nutritional dataset\n with @StarTrek_Lt Contribution.\n Tidytuesday Week-52 2021 · Abdoul ISSA BIDA.",
                       fontfamily = "Verlag", fontface = "bold", color = "white", hjust = 1, vjust = 1, size = 6.5) + 
  theme(
     text = element_text(color = "white"),
     plot.title = element_text(family = "Verlag", size = rel(4), face = "bold", margin = margin(t = .25, b = .25, unit = 'cm')),
...
ggsave(filename = glue::glue("{path}.png"), width = 9, height = 12, device = ragg::agg_png)
