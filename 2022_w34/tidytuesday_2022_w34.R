...
(daily_releases_plot <- daily_releases |> 
  ...
  labs(
    subtitle = "The number of transitions in a dense integrated circuit (IC)\n doubles about every two years, improving
    preformances both on CPUs and GPUs.",
    x = "Release date",
    y = "Nb releases\n in millions"
  ) + 
  ...
  theme(
    ...
    strip.text = element_text(size = rel(3), family = "UEFA Supercup", face = "bold", margin = margin(b = .5, unit ="cm")),
