...
# Plot --------------------------------------------------------------------
  ggplot() + 
    ...
    ggrepel::geom_text_repel(data = goal_lines_df, aes(x = minute + 0.5, 
    ...
    geom_point_svg(data = goal_lines_df, aes(x = minute + 0.5, y = goal_line_end),
                   svg = ball_svg,
                   size = 3.5) +
...
(final_plot <- half_time_plot(TRUE) +
    ...
    plot_annotation(title = glue::glue("**{toupper(home_team)} <span style=\"font-size: 45pt;color: {home_team_color};\">|</span> <img src='{home_team_logo}' width='35'/> {home_score} - {away_score} <img src='{away_team_logo}' width='35'/><span style=\"font-size: 45pt; color: {away_team_color};\">|</span> {toupper(away_team)}**<br/><br/><span style=\"font-size: 8pt;\">{subtitle}</span>"),
...
ggsave(here::here("Soccer/momentum.png"), width = 9.0, height = 6.75, dpi = 240, device = ragg::agg_png)
