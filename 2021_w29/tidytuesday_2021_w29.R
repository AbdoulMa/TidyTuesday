...
if (!dir.exists(here::here("2021_w29","networks_logos"))) {
  fs::dir_create(here::here("2021_w29","networks_logos"))
}

# Logos tibble with wikimedia urls
logos <- tibble(
  network = c("Cartoon Network","The CW","The WB","ABC","Boomerang","CBS"),
  url = paste0(wikimedia, c("1/1c/Cartoon_Network_logo.svg/800px-Cartoon_Network_logo.svg.png",
                             "4/4c/The_CW.svg/800px-The_CW.svg.png",  
                             "0/02/Logo_of_The_WB.svg/248px-Logo_of_The_WB.svg.png",
                             "5/54/American_Broadcasting_Company_Logo.svg/512px-American_Broadcasting_Company_Logo.svg.png",
                             "3/35/Boomerang_2014_logo.svg/300px-Boomerang_2014_logo.svg.png",
                             "e/ee/CBS_logo_(2020).svg/799px-CBS_logo_(2020).svg.png"
  )), 
  name = paste0(str_replace_all(str_to_lower(network)," ","_"),".png")
)


# Logos downloader function
download_logo <- function(url, dest_file) {
  if (!file.exists(here::here(dest_file))) {
    download.file(
      url = url,
      destfile = here::here(dest_file)
    )
  }
}

# Download logos in a specifical folder
logos %>% 
  select(url, name) %>% 
  pwalk(~download_logo(.x, paste0("2021_w29/networks_logos/",.y)))

scoobydoo_series <- scoobydoo %>% 
  left_join(logos, by = "network") %>% 
  mutate(imdb = parse_number(imdb),
         logo_path = here::here("2021_w29/networks_logos",name) ,
         national_tv = case_when(
           network %in% c("ABC", "The CW","CBS") ~ TRUE,
           TRUE ~ FALSE),
         network_logo = glue::glue("<img src='{logo_path}' width='30'/>")
         ) %>%  
...
(p2 <- scoobydoo_epi_motives %>% 
    ...
    geom_text(data = filter (scoobydoo_epi_motives, motive == "Competition"),aes(label =  glue::glue("{round(prop,3)*100}%")),
              position = position_fill(), hjust = 1.2,  family = "Inconsolata", 
              size = rel(4.5),color = "white", fontface = "bold")  +
    scale_x_continuous(
      expand = expansion(mult = 0), 
      breaks= seq(0,1, by = .2),
